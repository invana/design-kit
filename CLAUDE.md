...
- Organize stories under these top-level sections in `apps/storybook/stories/`: `ui-kit/`, `form-builder/`, `data-tables/`, `themes/` (for theme stories), and `others/` (catch-all for anything that doesn't fit). Set each story's `title` to match its section, e.g. `"UI Kit/Button"`, `"Form Builder/TextInput"`, `"Data Tables/DataTable"`, `"Themes/AppV2"`, `"Others/Palette"`.
